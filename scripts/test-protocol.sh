#!/usr/bin/env bash
# Host ↔ phone wire checks without glasses. Run while face·chat relay is connected.
set -euo pipefail
...
echo "== status =="
curl -sf "$HOST/face-chat/status" | head -c 500
echo

MSG="protocol-test $(date +%H:%M:%S)"
THREAD="$(curl -sf "$HOST/face-chat/status" | python3 -c "
import json,sys
...
if adb devices 2>/dev/null | awk 'NR>1 && $2=="device"{ok=1} END{exit !ok}'; then
  ...
  adb logcat -d -t 200 | grep -E "RelayClient|HudPresenter|hud_yank|RelayService" | tail -20 || echo "(no matching log lines — open face·chat on phone)"
fi
