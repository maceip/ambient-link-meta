// Settings + status screen. Mirrors phone-android/MainActivity — pairing button, relay
// URL field, connection status, list of known threads. No chat UI: the HUD is the chat UI.
import MWDATCore
import SwiftUI

private let defaultsKey = "fc.relayURL"

struct MainView: View {
  @Bindable var wearablesVM: WearablesViewModel
  @Bindable var daemon: DaemonState
  @State private var url: String = UserDefaults.standard.string(forKey: defaultsKey) ?? "wss://example.com/face-chat/ws"

  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 16) {
        Text("face·chat").font(.system(size: 22, weight: .bold))
