...
final class HudPresenter {
  ...
  // metadata until it does. Mirrors relay-android HudPresenter.LINK_WAIT_MS.
  ...
  func cancelIfFor(thread: String) {
    ...
    NSLog("[ambient-link] thread_busy %@ (HUD stays until dismissed)", thread)
  }
  ...
  private func openSessionAndPeek() {
    ...
    guard let candidate = ids.first(where: { wearables.deviceForIdentifier($0)?.supportsDisplay() == true }) else {
      NSLog("[ambient-link] no display-capable device known to SDK (\(ids.count) total)")
      return
    }

    let initial = wearables.deviceForIdentifier(candidate)?.linkState
    if initial == .connected {
      NSLog("[ambient-link] device already CONNECTED, opening session immediately")
      createSession(for: candidate)
      return
    }

    NSLog("[ambient-link] device link=\(String(describing: initial)) — waiting up to \(linkWaitMs)ms for CONNECTED")
    Task { [weak self] in
      guard let self else { return }
      if await self.awaitConnected(candidate) {
        NSLog("[ambient-link] device transitioned to CONNECTED — opening session")
        self.createSession(for: candidate)
      } else {
        let final = self.wearables.deviceForIdentifier(candidate)?.linkState
        NSLog("[ambient-link] timed out waiting for CONNECTED (final link=\(String(describing: final))); dropping yank")
      }
    }
  }
  ...
  // analog of relay-android's startWatchingLink metadata subscription.
  private func awaitConnected(_ id: DeviceIdentifier) async -> Bool {
    ...
    while DispatchTime.now().uptimeNanoseconds < deadline {
      ...
      if ls != last {
        NSLog("[ambient-link] link-watch: id=\(id) link=\(String(describing: ls))")
        last = ls
      }
  ...
  private func createSession(for id: DeviceIdentifier) {
    NSLog("[ambient-link] createSession via SpecificDeviceSelector id=\(id)")
    do {
      ...
      Task { [weak self] in
        for await st in s.stateStream() {
          guard let self else { return }
          NSLog("[ambient-link] session.state -> \(st)")
          if st == .started, self.display == nil { await self.attachDisplay(s) }
        }
      }
      try s.start()
    } catch {
      NSLog("[ambient-link] createSession failed: \(error)")
    }
  }
  ...
  private func attachDisplay(_ s: DeviceSession) async {
    do {
      ...
      NSLog("[ambient-link] addDisplay failed: \(error)")
    }
  }
  ...
  private func startDictating(_ y: AgentYank) async {
    ...
    dm.onError = { [weak self] msg in
      Task { @MainActor in
        self?.relay?.sendDictateAbort(thread: y.thread)
        NSLog("[ambient-link] dictate error: \(msg)")
        self?.closeSession()
      }
    }
    await dm.start()
